...
func GetAnalyseForVacanciesUser(vacancy models.Vacancy,
	...
	if err != nil {
		logger.Error.Printf("[aiService.GetAnalyseForVacanciesUser] Error marshalling kindernote\n\tUsers:%v\n\tError: %v", users, err)
		return nil, err
	}
	...
	var text = fmt.Sprintf(
		"Ниже приведены данные о пользователях:\n\n"+
			"1. Users: %v\n"+
			"Также прилагается вакансия: %v\n\n"+
			"Твоя задача — на основе предоставленных данных о пользователях "+
			"выбрать наиболее подходящего пользователя для вакансии."+
		...
		string(jsonUsers), string(jsonVacancy),
	)
	...
	if err != nil {
		logger.Error.Printf("[aiService.GetAnalyseForVacanciesUser] Error getting analyse: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Error.Printf("[aiService.GetAnalyseForVacanciesUser] Error reading body analyse: %v", err)
		return nil, err
	}

	var GeminiResp models.Gemini
	if err := json.Unmarshal(body, &GeminiResp); err != nil {
		logger.Error.Printf("[aiService.GetAnalyseForVacanciesUser] Error parsing body: %v", err)
		return nil, err
	}
