...
func InitRoutes(r *gin.Engine) *gin.Engine {
	...
	users := r.Group("/users", middlewares.CheckUserAuthentication, middlewares.CheckUserWorkerAndDepartment)
	{
		users.GET("", pagination.UsersWebSocket)
		users.GET("/:id", controllers.GetUserByID)
	}
	r.GET("/users/me", middlewares.CheckUserAuthentication, controllers.GetMyData)
	...
	// Тесты
	tests := r.Group("/tests", middlewares.CheckUserAuthentication)
	{
		tests.GET("/:id", controllers.GetTestByID)

		tests.POST("", middlewares.CheckUserWorker, controllers.CreateTest)
		tests.PUT("/:id", middlewares.CheckUserWorker, middlewares.CheckUserTest, controllers.UpdateTest)
		tests.DELETE("/:id", middlewares.CheckUserWorker, middlewares.CheckUserTest, controllers.DeleteTest)
		tests.POST("/find", controllers.GetTestsByTypeAndID)
	}

	testSubmissions := tests.Group("/submissions")
	{
		testSubmissions.GET("/:id", middlewares.CheckUserAuthentication, middlewares.CheckUserWorker, middlewares.CheckUserTest, controllers.GetSortedSubmissionsHandler)
